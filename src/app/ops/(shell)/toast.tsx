...
// Console toasts (EXPERIENCE.md): top-right, aria-live polite. Success
// auto-dismisses after 5s; failures persist until dismissed and can carry a
// retry affordance when the action is retryable.
        ...
        {toasts.map((toast) => (
          <div
            ...
            className={`flex items-start gap-3 rounded-lg border bg-popover p-3 text-sm shadow-lg ${
              toast.kind === "error" ? "border-status-critical/60" : "border-status-healthy/60"
            }`}
          >
            <span
              aria-hidden="true"
              className={`mt-1 size-2 shrink-0 rounded-full ${
                toast.kind === "error" ? "bg-status-critical" : "bg-status-healthy"
              }`}
            />
