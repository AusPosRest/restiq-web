// Server-side pass-through to the backend's /guest/v1 API. The guest JWT
// lives in an httpOnly cookie (never readable client-side), so browser code
// calls /qr/api/* and this handler attaches the Authorization header. The
// backend guard remains the enforcement point - this adds a credential, not
// authz. Mirrors src/app/pos/api/[...path]/route.ts (AD-4/AD-13, extended to
// the guest realm). No screens call this yet in this story (welcome/session
// PIN use their own auth/start and auth/join routes, which have no session
